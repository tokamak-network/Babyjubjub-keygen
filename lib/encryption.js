...
function test(){
  ...
  let C1 = M.add(K.mult(r)); //C1 = M + r*K
  let C2 = G.mult(k); //C2 = r*G

  //decryption

  let decM = C1.add(C2.mult(k).neg()); // M = C1 - k*C2

  console.log("C2.neg().x : ", C2.mult(k).neg().x.n.toFixed());
  console.log("C2.x       : ", C2.mult(k).x.n.toFixed());
